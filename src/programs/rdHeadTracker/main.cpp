...
int main(int argc, char *argv[]) {
    ...
    if(rf.check("robot") && !rf.check("help")) {  // Unless we have no robot, or just asking for help, we'll need to check connectivity.
        std::cout << "--------------------------------------------------------------" << std::endl;
        std::cout << "rdHeadTracker checking for yarp network... ";
        fflush(stdout);
        if (!yarp.checkNetwork()) {
            std::cerr << "[fail]" << std::endl
                 << "rdHeadTracker found no yarp network (try running \"yarpserver &\"), bye!" << std::endl;
            return -1;
        } else std::cout << "[ok]" << std::endl;
    }

    rdheadtracker::RdHeadTracker mod;  // This is the rdClient program main class.
