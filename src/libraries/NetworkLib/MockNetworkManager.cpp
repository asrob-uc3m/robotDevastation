...
bool rd::MockNetworkManager::sendPlayerHit(const Player & player, int damage)
{
    ...
    if (!logged_in)
    {
        yCError(RD_NET) << "Not logged in, id:" << id;
        return false;
    }
    ...
    {
        yCError(RD_NET) << "Target does not exist, id:" << id;
        return false;
    }
...
bool rd::MockNetworkManager::login()
{
    ...
    if (logged_in)
    {
        yCError(RD_NET) << "Already logged, id:" << loginId;
        return false;
    }
...
bool rd::MockNetworkManager::setLoggedIn(bool logged_in)
{
    ...
    {
        yCWarning(RD_NET) << "Already in that state:" << logged_in;
        return false;
    }
}
